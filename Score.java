...
public class Score extends JPanel implements MouseListener
{
    ...
    public void mousePressed(MouseEvent e)
    {
        ...
        if (e.getX()<430&&e.getX()>50 && e.getY()<430&&e.getY()>50)
        {
            ...
            g.drawOval (e.getX()-90, e.getY()-90, 200, 200);
            g.drawOval (e.getX()-65, e.getY()-65, 150, 150);
            g.drawOval (e.getX()-40, e.getY()-40, 100, 100);
            g.setColor(colour(cand+1));
            //g.drawRect (e.getX()-80, e.getY()-80, 175, 175); Uncomment this line to use rectangles which are 100% accurate
            Rectangle rect1 = new Rectangle(e.getX()-80, e.getY()-80, 175, 175);
            Rectangle rect2 = new Rectangle(e.getX()-55, e.getY()-55, 125, 125);
            Rectangle rect3 = new Rectangle(e.getX()-30, e.getY()-30, 75, 75);
            for (int i=0; i<vLoc.size();)
            {
                int x=vLoc.get(i);
                int y=vLoc.get(i+1);
                Rectangle voRec=new Rectangle(x, y, 5, 5);
                if(voRec.intersects(rect3))
                {
                    count+=3;
                    g.setColor(colour(cand+1));
                    g.fillOval(x, y, 5, 5);
                }
                if(voRec.intersects(rect2))
                {
                    count+=2;
                    g.setColor(colour(cand+1));
                    g.fillOval(x, y, 5, 5);
                }
                if(voRec.intersects(rect1))
                {
                    count++;
                    g.setColor(colour(cand+1));
                    g.fillOval(x, y, 5, 5);
